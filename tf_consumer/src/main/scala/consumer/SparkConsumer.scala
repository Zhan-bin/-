...
object SparkConsumer {
  def main(args: Array[String]): Unit = {
    ...
    val kafkaLineDStream = KafkaUtils.createDirectStream[
      ...
      .map(_._2) //取出来的数据是键值对的形式，且键值为null，只需要拿出value。

    //解析json字符串,最终得到的event的形式是：{"0001" -> "57"}
    val event = kafkaLineDStream.map(line => {
      //使用fastjson解析当前事件中封装的数据信息
      val lineJavaMap = JSON.parseObject(line, new TypeReference[java.util.Map[String, String]](){})//将json转换成map对象
    ...
    val sumOfSpeedAndCount = event
      ...
      (t1: (Int, Int), t2: (Int, Int)) => (t1._1 + t2._1, t1._2 + t2._2),//reducefunction
      Seconds(60),//窗口大小
      Seconds(60)) //滑动间隔
